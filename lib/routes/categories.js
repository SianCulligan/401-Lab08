...
const logCategories = (req, res, next) => {
  console.log('In the category route');
  next();
};

router.use(logCategories);
...
router.put('/:id', async (request, response, next) => {
  let modifiedRecord = await CategoryModel.update(request.params.id, request.body);
...
router.delete('/:id', async (request, response, next) => {
  let modifiedRecord = await CategoryModel.delete(request.params.id);
  response.send(modifiedRecord);
});
